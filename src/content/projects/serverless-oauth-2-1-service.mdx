...
My initial workaround involved redundant storage (storing entities by both ID and a user-prefixed key) to enable `list` operations based on username.
This approach quickly became cumbersome, increasing complexity during updates/deletions and relying on costly `list` operations.
